...
function createDiagram(svgSingleDiagram) {
  ...
  svgSingleDiagram
    ...
    .attr("dy", -10)
    .style("font-size", "1rem")
    .text("Network Characteristics: V = " + Voltage.toFixed(1) + " kV, S base = " + Sb.toFixed(1) + " MVA, Z base = " + Z_b.toFixed(2) + " Ω"+ ", fault at " +distanceToFault.toFixed(2) + "% of Line L ("+ lineLength.toFixed(1) + " miles)")
    .attr("id", "Basetext");  
...
function generateLine(x) {
  ...
  svgSingleDiagram_g.append("line")
    ...
    .attr("x2", x)
    .attr("y2", y2)
    .attr("transform", "rotate(45," + x + "," + yMin + ")") // 45° rotation
    .attr("stroke", "grey")
    .attr("stroke-width", 2);
}
